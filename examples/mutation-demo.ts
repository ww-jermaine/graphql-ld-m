...
const SAMPLE_REVIEWS = [
  {
    ...
    rating: 5,
    reviewText:
      'Exceptional performance and build quality. The M3 chip is incredibly fast for development work.',
  ...
  try {
    ...
    for (const product of SAMPLE_PRODUCTS) {
      ...
      try {
        ...
        console.log(`   ✅ Created: ${result.data ? product.id : 'Unknown ID'}`);
    ...
    try {
      ...
      products.forEach((product: any, index: number) => {
        console.log(
          `   ${index + 1}. ${product.name} - ${product.currency}${product.price} (${product.brand})`
        );
      });
    ...
    try {
      await client.mutate({ query: updateMutation });
      console.log('   ✅ Product updated successfully');

      // Verify the update
      const verifyResult = await client.query({ query: readQuery });
      const products = Array.isArray(verifyResult.data) ? verifyResult.data : [];
      const updatedProduct = products.find(
        (p: any) => p.id === `http://example.org/${updateTarget.id}`
      );
    ...
    for (const review of SAMPLE_REVIEWS) {
      ...
      try {
        ...
        console.log(`   ✅ Review created: ${result.data ? review.id : 'Unknown ID'}`);
    ...
    try {
      ...
      reviews.forEach((review: any, index: number) => {
        ...
        console.log(
          `   ${index + 1}. ${review.rating}⭐ for ${productInfo?.name || 'Unknown Product'}`
        );
    ...
    try {
      ...
      const productsWithReviews = Array.isArray(complexResult.data) ? complexResult.data : [];
      ...
      productsWithReviews.forEach((product: any, index: number) => {
        ...
        reviews.forEach((review: any, reviewIndex: number) => {
          console.log(
            `         ${reviewIndex + 1}. ${review.rating}⭐ - ${review.reviewText.substring(0, 40)}...`
          );
        });
      });
    } catch (error: any) {
    ...
    try {
      await client.mutate({ query: deleteReviewMutation });
    ...
    console.log(
      '      - Update: curl -X POST http://localhost:7200/repositories/graphql-demo/statements'
    );
...
async function verifyGraphDBConnection(): Promise<void> {
  try {
    ...
    const queryResponse = await fetch(
      'http://localhost:7200/repositories/graphql-demo?query=SELECT%20*%20WHERE%20%7B%20%3Fs%20%3Fp%20%3Fo%20%7D%20LIMIT%201'
    );
